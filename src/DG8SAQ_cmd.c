	...
	switch (type)
	{
		case 0x30:
			...
			case 0x31:								// Write the frequency subtract multiply to the eeprom
				...
				{
					cdata.FreqSub = Buf32[0];
					cdata.FreqMul = Buf32[1];
			...
			case 0x31:								// Write the frequency subtract multiply to the eeprom
				if (len == 2*sizeof(uint32_t))
				{
					cdata.BandSub[wIndex & 0x0f] = Buf32[0];
					cdata.BandMul[wIndex & 0x0f] = Buf32[1];
					flashc_memset32((void *)&nvram_cdata.BandSub[wIndex & 0x0f], Buf32[0], sizeof(uint32_t), TRUE);
					flashc_memset32((void *)&nvram_cdata.BandMul[wIndex & 0x0f], Buf32[1], sizeof(uint32_t), TRUE);
				}
				break;
			#endif
