...
class SDR1000
{
	 ...
	 */
	double FTW2Freq(uint64 ftw);

	/// Changes a frequency in MHz to a tuning word
	/** \param freq Frequency in MHz to convert
	 *  \return 48-bit frequency tuning word
	 */
	uint64 Freq2FTW(double freq);

public:
	/// Constructor
	SDR1000(char* name, BOOLEAN rfe, BOOLEAN pa, BOOLEAN usb, uint16 lpt_addr);
	 ...
	 */
	void SetSpurReductionMask(uint64 mask);
